...
render_page_header(
    ...
    "Configure the API, excipient search space, solver settings, and feasibility constraints, "
    "then request an optimised formulation recommendation from the DM² optimisation pipeline.",
...
with left_col:
    ...
    with st.container(border=True):
        ...
        selected_objectives = st.multiselect(
            "Objective functions",
            options=avail_objectives,
            default=[o for o in objective_defaults if o in avail_objectives],
            help="Select one for single-objective GA, two or more for NSGA-II.",
        )
        if selected_objectives:
            st.caption(f"Mode: {objective_mode(selected_objectives)}")
        else:
            st.caption("No objectives selected — the backend will use its configured defaults.")
        ...
        constraint_df = st.data_editor(
            ...
            column_config={
                "name": st.column_config.SelectboxColumn(
                    ...
                    required=True,
                    help="Choose from the available constraint types.",
                ),
                "threshold": st.column_config.NumberColumn(
                    "Threshold", step=0.01, format="%.4f", required=True
                ),
            },
        )
